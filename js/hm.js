(function() {
	var c = {
		id : "08de700479a393e55cc18d2df92b23bc",
		dm : ["jinrong.baidu.com"],
		etrk : [],
		...
		(function() {
			if (!d)
				if ( d = l, "complete" === document.readyState)
					...
					document.addEventListener("DOMContentLoaded", f, p), window.addEventListener("load", a, p);
	...
	var ba = navigator.cookieEnabled, ca = navigator.javaEnabled(), da = navigator.language || navigator.browserLanguage || navigator.systemLanguage || navigator.userLanguage || "", ea = window.screen.width + "x" + window.screen.height, fa = window.screen.colorDepth;
	var G;
	function H() {
		if (!G)
			try {
				G = document.createElement("input"), G.type = "hidden", G.style.display = "none", G.addBehavior("#default#userData"), document.getElementsByTagName("head")[0].appendChild(G)
			} catch(a) {
				return p
			}
		return l
	...
	function ga(a, b, d) {
		...
		try {
			window.localStorage ? ( b = e.getTime() + "|" + b, window.localStorage.setItem(a, b)) : H() && (G.expires = e.toUTCString(), G.load(document.location.hostname), G.setAttribute(a, b), G.save(document.location.hostname))
		} catch(f) {
		}
	}
	...
	function ma(a) {
		if (window.localStorage) {
			...
			try {
				return G.load(document.location.hostname), G.getAttribute(a)
			} catch(e) {
			}
		return n
	};
						...
						if ( a = a[1], D(a, "Object")) {
							var h = function(a) {
								...
								if ( {}.hasOwnProperty.call(a, b)) {
									var d = a[b];
									D(d, "Object") || D(d, "Array") ? h(d) : a[b] = String(d)
								}
							};
							h(a);
							this.a.api |= 16;
	...
	function ta() {
		...
		if (a)
			for (var a = a.split(","), b = 0, d = a.length; b < d; b++)
				C(Q + "//" + decodeURIComponent(a[b]).replace(/^https?:\/\//, ""), function(a) {
					S(a)
				})
	}
		...
		try {
			...
			if (document.referrer) {
				for (var m = 0, ua = c.se.length; m < ua; m++)
					if (RegExp("(^|\\.)" + c.se[m][1].replace(/\./g, "\\.")).test(A(document.referrer))) {
						var y = z(document.referrer, c.se[m][2]) || "";
						if (y || !(2 != c.se[m][0] && 14 != c.se[m][0] && 17 != c.se[m][0])) {
							1 == c.se[m][0] && -1 < document.referrer.indexOf("cpro.baidu.com") && ( y = "");
							a.a.se = c.se[m][0];
							for (var za = a.a, q = c.se[m], J = 0, Aa = 2 == q[3] ? q[1] + "\\/" : "", Ba = 1 == q[3] ? "\\." + q[1] : "", V = q[4].split(","), q = 0, Ca = V.length; q < Ca; q++)
				...
				y = p;
				T(document.referrer) && T(document.location.href) ? y = l : ( J = A(document.referrer), y = U(J || "", document.location.hostname));
			...
			if ("" != c.icon) {
				...
				switch(w[1]) {
					case "swf":
						var ia = w[2], ja = w[3], M = "s=" + Y, u = "HolmesIcon" + K;
						...
						break;
					case "gif":
						v = '<a href="' + Y + '" target="_blank"><img border="0" src="' + Z + '" width="' + w[2] + '" height="' + w[3] + '"></a>';
						break;
					default:
			...
			if (N && Da.test(N) && Ea) {
				...
				O.setAttribute("src", Q + "//" + c.js + z(N, "jn") + "." + z(N, "sx") + "?" + a.a.rnd);
			...
			a.l && a.l();
			if (c.rec || c.trust)
				a.a.nv ? (a.g = encodeURIComponent(document.referrer), window.sessionStorage ? s("Hm_from_" + c.id, a.g) : ga("Hm_from_" + c.id, a.g, 864E5)) : a.g = (window.sessionStorage ? t("Hm_from_" + c.id) : ma("Hm_from_" + c.id)) || "";
			a.n && a.n();
			a.o && a.o();
			a.e = new ya;
			r(window, "beforeunload", xa(a));
			var x = window._hmt;
			if (x && x.length)
			...
			a = [], a.push("si=" + c.id), a.push("n=" + encodeURIComponent(la.name)), a.push("m=" + encodeURIComponent(la.message)), a.push("r=" + encodeURIComponent(document.referrer)), C(Q + "//hm.baidu.com/hm.gif?" + a.join("&"))
		}
	}
	...
	function sa() {
		...
		try {
			if (aa(a, "", {
				...
				try {
					G.load(document.location.hostname), G.removeAttribute(a), G.save(document.location.hostname)
				} catch(b) {
				}
		} catch(d) {
		}
	...
	function ya() {
		...
		this.i = 0;
		"object" == typeof document.onfocusin ? (r(document, "focusin", W(this)), r(document, "focusout", W(this))) : (r(window, "focus", W(this)), r(window, "blur", W(this)))
	}
